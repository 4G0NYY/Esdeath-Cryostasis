...
// against and the same host the loader installer comes from. Using it keeps the installer
// off third-party mod hosts and their rate limits.
...
// latestFabricApi resolves the newest Fabric API build for a Minecraft version, returning
// the version, its jar URL, and the filename to save it under.
func latestFabricApi(mcVersion string) (version, url, filename string, err error) {
	...
	// The '+' stays literal: it separates path segments' version here, and only means a
