...
// findFabricVersion returns the installed Fabric version id for the given Minecraft
// version, or "" when Fabric is not installed for it. Fabric names its version folders
...
func installFabric(mcDir, mcVersion string) error {
	java, err := findJava(mcDir)
	if err != nil {
		return err
	}
	info("using java at %s", java)
	...
	info("downloading Fabric installer %s", chosen.Version)
